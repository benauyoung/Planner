...
export const SchemaNode = memo(function SchemaNode({ id, data }: NodeProps) {
  ...
  const schemaType = nodeData.schemaType || 'other'
      ...
      <div className="p-3">
        <div className="flex items-center gap-2 mb-2">
          <Badge className={cn('text-[10px] px-1.5 py-0', config.badgeClass)}>
            ...
            {SCHEMA_TYPE_LABELS[schemaType] || config.label}
          </Badge>
        ...
        {description ? (
          ...
          <div className="flex flex-col items-center justify-center py-4 border border-dashed rounded-md text-muted-foreground">
            <Braces className="h-6 w-6 mb-1.5 opacity-50" />
            <span className="text-xs">Click to define schema</span>
          </div>
        )}
      </div>
