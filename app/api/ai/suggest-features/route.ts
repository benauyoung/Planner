...
You MUST respond with ONLY valid JSON matching this exact schema. No markdown, no explanation, no code fences — just the raw JSON object:
{
...
const userPrompt = (description: string, projectType: string) =>
  `Project description: ${description}\nProject type: ${projectType}\n\nSuggest 8-12 specific features and 2-4 tailored planning questions for this project.`

async function callClaude(description: string, projectType: string) {
  const client = getAnthropicClient()
  const result = await client.messages.create({
    model: CLAUDE_MODEL,
    max_tokens: 2048,
    system: SYSTEM_PROMPT + JSON_SCHEMA_INSTRUCTION,
    messages: [{ role: 'user', content: userPrompt(description, projectType) }],
  })
...
async function callGemini(description: string, projectType: string) {
  ...
  const result = await model.generateContent(userPrompt(description, projectType))
...
export async function POST(req: Request) {
  try {
    ...
    if (isAnthropicConfigured()) {
      try {
        const parsed = await callClaude(description, projectType)
    ...
    // Gemini fallback
    const parsed = await callGemini(description, projectType)
    return NextResponse.json(parsed)
  } catch (error) {
